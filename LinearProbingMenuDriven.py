'''Menu driven code for reading multiple data from user and to perform search and delete operation as per value
intered by the user'''
...
option=1
while(option==1):
    num=int(input("Enter an number to insert into Hash table"));
    insertHash(num);
    display(); # Print hash table along with data.
    option=int(input("Enter 0 to Exit and 1 to read more data=>"));
    
num=int(input("Enter number to search in Hash table"));
search(num) #searching operation in hash table

num=int(input("Enter number to deleted from Hash table"));
